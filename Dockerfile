...
COPY --from=docker.io/alpine/helm:3.15.1                /usr/bin/helm                    /usr/local/bin/helm
...
COPY --from=ghcr.io/kyverno/kyverno-cli:v1.12.2         /ko-app/kubectl-kyverno          /usr/local/bin/kyverno
